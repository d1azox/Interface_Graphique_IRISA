...
class SegNet(nn.Module):
    def __init__(self,in_channel=3,class_num = 12):
        ...
        self.vgg16_kernel_num = [in_channel,64,128,256,512,512]
...
class SegNetLevel(nn.Module):
    def __init__(self,in_channel=3,class_num = 12,  lev=5):
        super(SegNetLevel, self).__init__()
        self.class_num = class_num
        self.level=lev
        ...
        #Encoder
        self.vgg16_block = [2,2,3,3,3]
        self.vgg16_block =self.vgg16_block[0:self.level]
        self.vgg16_kernel_num = [in_channel,64,128,256,512,512]
        self.vgg16_kernel_num =self.vgg16_kernel_num[0:self.level+1]
        ...
        #Decoder
        self.decoder_block = [3, 3, 3, 2, 2]
        self.decoder_block = self.decoder_block[5-self.level:5]
        
        self.decoder_kernel_num = [512, 512, 256, 128, 64, self.class_num]
        self.decoder_kernel_num = self.decoder_kernel_num[5-self.level:6]
...
class SegNet2Encoder(nn.Module):
    def __init__(self,in_channel=3,class_num = 12):
        super(SegNet2Encoder, self).__init__()
        self.class_num = class_num
        
        self.in_channels=in_channel
        #Encoder
        self.vgg16_block = [2,3]
        self.vgg16_kernel_num = [in_channel,64,256]
        for i in range(len(self.vgg16_block)):
            block=[]
            for j in range(self.vgg16_block[i]):
                if j == 0:
...
#            block.append(nn.MaxPool2d(kernel_size=2,stride=2,return_indices=True))
        ...
        #Decoder
        self.decoder_block = [  3,  2]
        self.decoder_kernel_num = [256, 64, self.class_num]
        for i in range(len(self.decoder_block)):
            block = []
            for j in range(self.decoder_block[i]):
                if j != self.decoder_block[i]-1:
...
#        xe4, idx4 = self.vgg16_block4(xe3)
#        xe4_size = xe4.size()
        xe2  = self.vgg16_block2(xe1)
#        xd2 = F.max_unpool2d(xe2, idx2, kernel_size=2, stride=2, output_size=xe1_size)
        xd2 = self.decoder_block2(xe2)
#        xd4 = F.max_unpool2d(xd5, idx4, kernel_size=2, stride=2, output_size=xe3_size)
#        xd4 = self.decoder_block4(xd4)
#        xd3 = F.max_unpool2d(xd4, idx3, kernel_size=2, stride=2, output_size=xe2_size)
#        xd3 = self.decoder_block3(xd3)
...
#        xd2 = self.decoder_block2(xd2)
#        xd1 = F.max_unpool2d(xd2, idx1, kernel_size=2, stride=2, output_size=x_size)
        xd1 = self.decoder_block1(xd2)

        return xd1


class SegNet2EncoderOLD(nn.Module):
    def __init__(self,in_channel=3,class_num = 12):
        super(SegNet2Encoder, self).__init__()
        self.class_num = class_num
        ...
        #Encoder
        self.vgg16_block = [2,3]
        self.vgg16_kernel_num = [in_channel,64,512]
        for i in range(len(self.vgg16_block)):
            block=[]
            for j in range(self.vgg16_block[i]):
                if j == 0:
        ...
        #Decoder
        self.decoder_block = [  3,  2]
        self.decoder_kernel_num = [512, 64, self.class_num]
        for i in range(len(self.decoder_block)):
            block = []
            for j in range(self.decoder_block[i]):
                if j != self.decoder_block[i]-1:
